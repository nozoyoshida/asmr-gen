...
3.  **インテリジェントな空間演出プランニング (Context & Audio Analysis)**
    *   **本アプリの肝となる機能です。** Gemini 2.5 Proが、**脚本の文脈**と**音声のタイミング（間やポーズ）**の両方を分析し、没入感を最大化するための音源の位置、距離、響きを時系列で計画します。
...
4.  **空間演出プラン生成**: 空間演出エージェントが**脚本とモノラル音声**を分析し、音響プラン（JSON）を作成。
...
*   **役割**: **脚本の文脈**と、**音声のタイミング（間やポーズ）**を分析し、没入感を最大化する音響設計を行う。ASMR-GENの肝となる機能。
